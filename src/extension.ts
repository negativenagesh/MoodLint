...
/**
 * Get the Python executable path - checks for venv first, then falls back to system Python
 */
function getPythonPath(extensionPath: string): string {
  // Check for virtual environment in extension directory
  const venvPaths = [
    path.join(extensionPath, ".venv", "bin", "python"), // macOS/Linux venv
    path.join(extensionPath, ".venv", "Scripts", "python.exe"), // Windows venv
    path.join(extensionPath, "venv", "bin", "python"), // Alternative venv name
    path.join(extensionPath, "venv", "Scripts", "python.exe"),
  ];

  for (const venvPath of venvPaths) {
    if (fs.existsSync(venvPath)) {
      console.log(`[Extension] Found Python at: ${venvPath}`);
      return venvPath;
    }
  }
  ...
  console.log(`[Extension] Using system Python: ${systemPython}`);
  return systemPython;
}

/**
 * Start Python camera application with specified model type
 */
function startPythonCameraApp(
  ...
  return new Promise((resolve, reject) => {
    ...
    console.log(`[Extension] Starting Python camera app with model type: ${modelType}`);
    console.log(`[Extension] Script path: ${scriptPath}`);
    ...
    try {
      ...
      // Read .env file to get OPENAI_API_KEY
      ...
      try {
        if (fs.existsSync(envPath)) {
          ...
          if (match) {
            ...
            console.log(`[Extension] Found OPENAI_API_KEY in .env`);
          }
        }
      } catch (error) {
        console.log(`[Extension] Could not read .env file: ${error}`);
      }

      // Spawn the process without shell for better GUI compatibility
      // Using detached option to ensure the process can open windows
      // Pass model type as command line argument
      cameraProcess = childProcess.spawn(
        pythonPath,
        [scriptPath, sessionId, modelType],
        {
          env: {
            ...process.env,
      ...
      const timeout = setTimeout(() => {
        console.log(
          "[Extension] Camera app startup timeout - assuming it started anyway",
        );
        resolve();
      }, 5000);
        ...
        try {
          ...
          if (output) {
            try {
              ...
              if (result.error) {
                console.error(
                  `[Extension] Python reported error: ${result.error}`,
                );
      ...
      setTimeout(() => {
        if (cameraProcess && cameraProcess.pid) {
          console.log(
            `[Extension] Camera app started with PID: ${cameraProcess.pid}`,
          );
          resolve();
        }
      }, 1000);
...
async function launchLstmPrediction() {
  try {
    ...
    if (imageCount < 1) {
      vscode.window.showWarningMessage(
        'No mood images found. Please use "Go with mood debug" to capture your mood first.',
      );
      if (moodlintPanel) {
    ...
    console.log(
      `[Extension] Launching LSTM prediction UI for directory: ${imagesDir}`,
    );
    ...
    const lstmProcess = childProcess.spawn(pythonPath, [
      lstmPopupUiPath,
      imagesDir,
    ]);
 ...
 */
async function generateFutureMoodVisualization(currentMood: string, backend: string = 'local') {
  try {
    // Check if we have a captured image to use
    if (!capturedImagePath) {
      vscode.window.showWarningMessage(
        'Please upload your image first by clicking "Go with mood debug"',
      );
      if (moodlintPanel) {
        moodlintPanel.webview.postMessage({
    ...
    console.log(
      `[Extension] Starting GAN popup for mood: ${currentMood} using image: ${capturedImagePath}`,
    );
    ...
    // Read .env for GEMINI_API_KEY if backend is gemini
    let geminiApiKey = "";
    if (backend === 'gemini') {
      try {
        ...
        if (fs.existsSync(envPath)) {
          ...
          if (match) {
            ...
            console.log(`[Extension] Found GEMINI_API_KEY in .env (Length: ${geminiApiKey.length})`);
          } else {
            console.log(`[Extension] GEMINI_API_KEY not matching regex in .env`);
          }
        }
      } catch (error) {
        console.log(`[Extension] Could not read .env for Gemini key: ${error}`);
      }
    }
    ...
    const outputImagePath = path.join(
      os.tmpdir(),
      `moodlint_gan_${Date.now()}.png`,
    );
    // Launch the GAN popup process
    const ganProcess = childProcess.spawn(
      pythonPath,
      [scriptPath, capturedImagePath, currentMood, outputImagePath, backend],
      {
        env: {
          ...process.env,
      ...
      try {
        ...
        if (output) {
          try {
            ...
            if (result.error) {
              console.error(
                `[Extension] GAN popup reported error: ${result.error}`,
              );
    ...
    vscode.window.showErrorMessage(
      `Error generating mood visualization: ${error instanceof Error ? error.message : String(error)}`,
    );
...
function createMoodlintPanel(context: vscode.ExtensionContext) {
  ...
  const iconPath = vscode.Uri.file(
    path.join(context.extensionPath, "MoodLint-logo", "logo.png"),
  );
  ...
  const imagePath = vscode.Uri.file(
    path.join(context.extensionPath, "MoodLint-logo", "cover.png"),
  );
  ...
  moodlintPanel.webview.html = getWebviewContent(
    stylesUri,
    scriptUri,
    imageUri,
    faviconUri,
  ...
  moodlintPanel.webview.onDidReceiveMessage(
    (message) => {
      ...
      if (!moodlintPanel) {
        console.error(
          "[Extension] moodlintPanel is undefined, cannot process message",
        );
        return;
      }
      switch (message.command) {
        ...
        case "startExternalCamera":
          // Launch the Python camera app with selected model type
          const modelType = message.modelType || "local"; // Default to local if not specified
          startPythonCameraApp(context, modelType)
            .then(() => {
              vscode.window.showInformationMessage(
                "External camera launched successfully",
              );
              moodlintPanel?.webview.postMessage({
        ...
        case "predictFutureMood":
          console.log(
            `[Extension] Predicting future mood based on: ${message.currentMood}`,
          );
          predictFutureMoodFromCurrent(message.currentMood);
          break;
        case "generateFutureMood":
          console.log(
            `[Extension] Generating future mood visualization based on: ${message.currentMood}`,
          );
          generateFutureMoodVisualization(message.currentMood, message.backend || 'local');
          break;
        default:
...
function processMoodFromPython(
  mood: string,
  confidence: number,
  filepath?: string,
  autoAnalyze?: boolean,
  ...
  console.log(
    `[Extension] Processing mood: ${mood} (${confidence}) from image: ${filepath || "unknown"}, autoAnalyze: ${autoAnalyze}`,
  );
  currentMood = mood;
  ...
  if (moodlintPanel) {
    ...
    const autoAnalyzeFromSettings = vscode.workspace
      .getConfiguration("moodlint")
      .get("autoAnalyzeAfterDetection", false);
    ...
    console.error(
      "[Extension] moodlintPanel undefined, cannot send moodDetected",
    );
  }
}
...
async function analyzeWithMood(mood: string, confidence: number, options: any) {
  ...
  try {
    ...
    console.log(
      `[Extension] Starting agent popup for mood: ${mood}, file: ${filename || "none"}`,
    );
          ...
          try {
            ...
            if (result.status === "complete" && result.result) {
              ...
              if (agentResult.success && agentResult.response) {
                ...
                const issues = extractIssuesFromResponse(
...
export function activate(extensionContext: vscode.ExtensionContext) {
  ...
  agentInterface.checkAgentInstallation().then((isInstalled) => {
    if (!isInstalled) {
      vscode.window.showWarningMessage(
        "MoodLint agent system not fully installed. Some features may not work.",
      );
    }
  });
